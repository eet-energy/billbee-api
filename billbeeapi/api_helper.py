    ...
    def serialize_array(key, array, formatting="indexed"):
        ...
        if isinstance(array[0], serializable_types):
            if formatting == "unindexed":
                tuples += [(f"{key}[]", element) for element in array]
            elif formatting == "indexed":
                tuples += [(f"{key}[{index}]", element) for index, element in enumerate(array)]
            ...
            if value is not None:
                if isinstance(value, list):
                    ...
                    if array_serialization == "csv":
                        url += "{}{}={}".format(seperator, key, ",".join(quote(str(x), safe="") for x in value))
            ...
            if isinstance(value, list):
                ...
                for key in value:
                    dictionary[obj._names[name]][key] = (
                        APIHelper.to_dictionary(value[key]) if hasattr(value[key], "_names") else value[key]
                    )
            else:
